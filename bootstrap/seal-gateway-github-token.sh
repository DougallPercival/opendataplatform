...
# Everything from the last document boundary before the placeholder SealedSecret is unchanged
# (Deployment/Service/RBAC/Certificate/Ingress); only the bitnami.com/v1alpha1 SealedSecret document
# at the bottom gets replaced — found by its own apiVersion line, the only one of those in this file.
boundary_line="$(grep -n '^apiVersion: bitnami\.com/v1alpha1$' "$GATEWAY_MANIFEST" | tail -1 | cut -d: -f1)"
[[ -n "$boundary_line" ]] || die \
  "Couldn't find the placeholder SealedSecret document in ${GATEWAY_MANIFEST} — has this already \
been run, or has that file's structure changed? Check it by hand before re-running."
keep_through=$((boundary_line - 1))

head -n "$keep_through" "$GATEWAY_MANIFEST" > "${work_dir}/new-gateway.yaml"
cat "${work_dir}/sealed.yaml" >> "${work_dir}/new-gateway.yaml"
mv "${work_dir}/new-gateway.yaml" "$GATEWAY_MANIFEST"
